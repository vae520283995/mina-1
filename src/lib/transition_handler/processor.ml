...
let process_transition ~logger ~trust_system ~verifier ~frontier
    ...
    ~block_or_header ~valid_cb ~precomputed_values =
  ...
  | `Header env -> (
      ...
      @@
      match
        Mina_block.Validation.validate_frontier_dependencies ~logger
          ~consensus_constants ~root_block ~is_block_in_frontier
          ~to_header:ident
